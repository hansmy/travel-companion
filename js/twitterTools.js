...
TwitterMarker.prototype.showTweet = function (a, article) {
    //e.target._latlng
    var popup = new TwitterPopUp(a.results,article);
    popup.setLatLng(new L.LatLng(article.lat, article.lng));
    popup.openOn(geoTools.map);

}

function TwitterPopUp(opt,article) {
    var strTweet= "<span>"+"<a href="+article.mobileurl+">"+article.title+"</a>"+"</span><br>";
    if (!opt || !opt.length) {
        this.setContent(strTweet+"No tweets related to this place");
    } else {
        this.count = opt.length;
        this.crnt = 0;
        this.id = TwitterPopUp.tweets.push(this);
        this.aTweets = opt;
        strTweet = strTweet+this.buildTwitterHTML(opt[0]);
