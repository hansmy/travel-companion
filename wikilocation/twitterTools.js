...
twitterTools.searchTwitter = function(article, position) {
	...
	if (!twitterTools.latlng || twitterTools.latlng && Math.abs(twitterTools.latlng.lat - article.lat) + Math.abs(twitterTools.latlng.lng - article.lng) > 0.0000005) {
		...
		var text = "<a href=" + article.url + ">" + article.title + "</a>";
...
function TwitterMarker(article) {
	...
	this.on('click', function(e) {
	 ...
	 $.ajax({
	 context : this,
	 url : 'http://localhost:3000/api/search/' + searchKeyWord + '&geocode=' + geocode + '&result_type=recent&rpp=10',
	 type : 'GET',
	 async : false,
...
TwitterPopUp.prototype.buildTwitterHTML = function(twt) {
	...
	if (twt.from_user_name) {
		str = "<span><img style='float: left' src='" + twt.profile_image_url + "' />" + "<b>" + twt.from_user_name + "</b><br/><a href ='http://twitter.com/" + twt.from_user + "'>@" + twt.from_user + "</a><br/> " + twt.location + "</span>" + "<p>" + twt.text + "</p>";

		if (this.count > 1) {

			str += "<span style='absolute; bottom: 0; right: 0px; width:80px'>";
			if (this.crnt != 0)
				str += "<a href='javascript:TwitterPopUp.tweets[" + (this.id - 1) + "].prev();'>&lt;</a> ";
			str += (this.crnt + 1) + " of " + this.count;
			if (this.crnt < (this.count - 1))
				str += "<a href='javascript:TwitterPopUp.tweets[" + (this.id - 1) + "].next();'>&gt;</a> ";
			str += "</span>"
		}
	} else {
